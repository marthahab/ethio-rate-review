...
export const Route = createFileRoute('/businesses/$id')({
  loader: async ({ params }) => {
    const response = await fetch(
      `http://localhost:3001/businesses/${params.id}`,
    )
...
function BusinessDetails() {
  ...
  async function handleSubmitReview(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      await createReview(business.id, rating, comment, token)
      setComment('')
      setRating(5)
      setShowForm(false)
      ...
      <section className="bg-[#174d3d] px-6 py-16 text-white lg:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-4 text-sm text-[#dce7df]">
            <span>Star {business.averageRating.toFixed(1)}</span>
            <span>
      ...
      <section className="px-6 py-12 lg:px-8">
        <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[2fr_1fr]">
          <div className="space-y-8">
                        {/* Business Image */}
            {business.imageUrl ? (
              <div className="h-72 overflow-hidden rounded-3xl bg-[#dfe8d9]">
                <img
            ...
            <div className="rounded-3xl bg-white p-8 shadow-sm">
              ...
              <p className="mt-4 leading-7 text-[#718078]">
                {business.description || 'No description has been added yet.'}
              </p>
            </div>

            <div className="rounded-3xl bg-white p-8 shadow-sm">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold text-[#173f35]">Reviews</h2>
              ...
              {showForm && (
                <form
                  ...
                  className="mt-6 space-y-4 rounded-2xl bg-[#f7f5ed] p-6"
                >
                  <div>
                    <label className="mb-2 block text-sm font-semibold text-[#173f35]">
                      Your Rating
                    </label>
                      ...
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button
                          ...
                          className={`text-2xl transition ${
                            star <= rating ? 'text-[#f4c84a]' : 'text-[#dfe4d8]'
                          }`}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      required
                      rows={4}
                      className="w-full rounded-xl border border-[#dfe4d8] bg-white px-4 py-3 text-sm outline-none transition focus:border-[#174d3d] focus:ring-2 focus:ring-[#174d3d]/10"
                    />
                  </div>
                  ...
                  <div className="flex gap-3">
                    <button
                      type="submit"
                      disabled={loading}
                      className="rounded-xl bg-[#174d3d] px-5 py-2.5 font-semibold text-white transition hover:bg-[#0f392d] disabled:opacity-60"
                    >
                    ...
                    <button
                      ...
                      className="rounded-xl px-5 py-2.5 font-semibold text-[#718078] transition hover:bg-[#eef1e8]"
                    >
                      Cancel
                    </button>
                  </div>
